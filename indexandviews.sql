...
e.job_title,
COALESCE(SUM(CASE WHEN at.activity_name = 'Lecture' THEN a.hours_allocated * at.factor END), 0) AS lecture_hours,
COALESCE(SUM(CASE WHEN at.activity_name = 'Tutorial' THEN a.hours_allocated * at.factor END), 0) AS tutorial_hours,
COALESCE(SUM(CASE WHEN at.activity_name = 'Lab' THEN a.hours_allocated * at.factor END), 0) AS lab_hours,
COALESCE(SUM(CASE WHEN at.activity_name = 'Seminar' THEN a.hours_allocated * at.factor END), 0) AS seminar_hours,
COALESCE(SUM(CASE WHEN at.activity_name = 'Overhead' THEN a.hours_allocated * at.factor END), 0) AS overhead_hours,
COALESCE(SUM(CASE WHEN at.activity_name = 'Admin' THEN a.hours_allocated * at.factor END), 0) AS admin_hours,
COALESCE(SUM(CASE WHEN at.activity_name = 'Exam' THEN a.hours_allocated * at.factor END), 0) AS exam_hours,
...
GROUP BY a.courseinstance_id, cl.course_code, cl.hp, ci.period, ci.year, a.employee_id, teacher_name, e.job_title;
...
GROUP BY pb.courseinstance_id, pb.course_code, pb.hp, pb.period, pb.year, pb.num_students, pb.total_planned_hours;
