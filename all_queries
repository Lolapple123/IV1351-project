...
(500001, 'PN900001', 1, 55000, 1, '[paris.carbone@example.com](mailto:paris.carbone@example.com)'),
(500004, 'PN900004', 2, 48000, 1, '[leif.lindback@example.com](mailto:leif.lindback@example.com)'),
(500009, 'PN900009', 2, 44000, 1, '[niharika.gauraha@example.com](mailto:niharika.gauraha@example.com)'),
(500010, 'PN900010', 3, 24000, 1, '[brian.karlsson@example.com](mailto:brian.karlsson@example.com)'),
(500011, 'PN900011', 4, 20000, 1, '[adam.west@example.com](mailto:adam.west@example.com)'),
(500100, 'PN901000', 2, 42000, 1, '[test.teacher@example.com](mailto:test.teacher@example.com)');
...
-- Course Instances
-- =========================
INSERT INTO CourseInstance(courselayout_id, course_code, version_number, study_period, year, num_students)
SELECT courselayout_id, course_code, 1, 'P2', 2025, 200 FROM CourseLayout WHERE course_code='IV1351';

INSERT INTO CourseInstance(courselayout_id, course_code, version_number, study_period, year, num_students)
SELECT courselayout_id, course_code, 1, 'P1', 2025, 150 FROM CourseLayout WHERE course_code='IX1500';

INSERT INTO CourseInstance(courselayout_id, course_code, version_number, study_period, year, num_students)
SELECT courselayout_id, course_code, 1, 'P1', 2025, 50 FROM CourseLayout WHERE course_code='IV1351';

INSERT INTO CourseInstance(courselayout_id, course_code, version_number, study_period, year, num_students)
SELECT courselayout_id, course_code, 1, 'P2', 2025, 40 FROM CourseLayout WHERE course_code='IX1500';
...
FROM CourseInstance ci
WHERE ci.study_period='P2' AND ci.year=2025
LIMIT 1;

-- IV1351 P2 Seminar
INSERT INTO Allocation(employee_id, courseinstance_id, hoursallocated)
SELECT emp.e_id, ci.courseinstance_id, 64
FROM CourseInstance ci
...
FROM CourseInstance ci
WHERE ci.study_period='P1' AND ci.year=2025
LIMIT 1;
...
SELECT
    ...
    ci.year,
    SUM(a.hoursallocated) AS total_hours  -- ta bort * at.factor om du inte har activitytype_id
FROM Allocation a
...
SELECT
    ...
    ci.study_period,
    ci.year,
    SUM((e.salary / 160.0) * a.hoursallocated) AS total_cost_sek
FROM Allocation a
JOIN Employee e ON a.employee_id = e.employee_id
JOIN CourseInstance ci ON a.courseinstance_id = ci.courseinstance_id
